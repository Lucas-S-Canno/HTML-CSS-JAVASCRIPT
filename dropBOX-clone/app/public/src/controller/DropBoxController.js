class DropBoxController {
    ...
    connectFirebase() {
        ...
        var firebaseConfig = {
            apiKey: "AIzaSyDd654yGsHYrQB2SmWQmAx30OXRACBdRJ4",
            authDomain: "dropbox-clone-6b9c2.firebaseapp.com",
            databaseURL: "https://dropbox-clone-6b9c2-default-rtdb.firebaseio.com",
            projectId: "dropbox-clone-6b9c2",
            storageBucket: "dropbox-clone-6b9c2.appspot.com",
            messagingSenderId: "961270970555",
            appId: "1:961270970555:web:e35230a60f7dcba5e5919e",
            measurementId: "G-P53SN7XPQ9"
          };
          // Initialize Firebase
    ...
    // delete para arquivos no firebase storage
    removeTask() {
        let promises = [];
    ...
    removeFile(ref, name){
        let fileRef = firebase.store().ref(ref).child(name);
    ...
    // delete para arquivos locais com informações no banco de dados
    // removeTask() {
    //     let promises = [];
    //     this.getSelection().forEach(li => {
    //         let file = JSON.parse(li.dataset.file);
    ...
    // uploadTask para upload de arquivos em nuvem (no caso firebase storage)
    uploadTask(files) {
        let promises = [];
        [...files].forEach(file => {
    ...
    // função uploadTask para guardar em servidor local
    // uploadTask(files) {
    //     let promises = [];
    //     [...files].forEach(file => {
    //         let formData = new FormData();
    ...
    initEventsLi(li) {
        ...
        li.addEventListener('dblclick', e => {
            ...
            switch (file.type) {
                ...
                default:
                    // window.open('/file?path=' + file.path); //Abrir arquivo local pelo banco de dados
                    window.open(file.path); //abrindo arquivos pelo firebase storage
            }
        });
            ...
            if (e.shiftKey) {
                ...
                if (firstLi) {
                    ...
                    lis.forEach((el, index) => {
                        ...
                        if (li === el) secondIndex = index;
                    });
                    let index = [firstIndex, secondIndex].sort();
                    lis.forEach((el, i) => {
